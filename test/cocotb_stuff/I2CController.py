...
class I2CController():
    ...
    def __init__(self, dut, CYCLES_PER_BIT: int, pp: bool = False, GL_TEST: bool = False):
        ...
        # FIXME despite the names self._sdascl this related to the old original mapping, this is
        #   transposed in the new mapping.  Fix these APIs and make it better/unittest to clear up
        #   this and allow reversal of returned bit-bus option
        ...
        # TODO fix this API so it does not matter the range is first/last
        first_bit = SCL_BITID if(SCL_BITID < SDA_BITID) else SDA_BITID
        last_bit = SDA_BITID if(SDA_BITID > SCL_BITID) else SCL_BITID
        self._sdascl_transpose = SCL_BITID > SDA_BITID
        #self._sdascl = self._sa_uio_in.register('uio_in', SCL_BITID, SDA_BITID) # with old mapping
        self._sdascl = self._sa_uio_in.register('uio_in', first_bit, last_bit)
        ...
        self._sdascl_out = self._sa_uio_out.register('uio_out', first_bit, last_bit)
        # uio_oe: This is peer OE
        self._sa_uio_oe = SignalAccessor(dut, 'uio_oe')	# FIXME pull from shared registry ?
        self._sdascl_oe = self._sa_uio_oe.register('uio_oe', first_bit, last_bit)

        if self._sdascl_transpose:
            self._sda_int_mask = 1 # new bitmask of _sdascl for sda part
            self._scl_int_mask = 2 # new bitmask of _sdascl for sda part # not used?
        else:
            self._sda_int_mask = 2 # old bitmask of _sdascl for scl part
            self._scl_int_mask = 1 # old bitmask of _sdascl for scl part # not used?
    ...
    def scl_raw(self, v: bool) -> None:
        ...
        self._sdascl.value = self.resolve_bits_state_str(sda, v, self._sdascl_transpose)
    ...
    def sda_raw(self, v: bool) -> None:
        ...
        self._sdascl.value = self.resolve_bits_state_str(v, scl, self._sdascl_transpose)
    ...
    def signal_to_string(self, signal) -> str:
        s = str(signal)
        # This changes based on which ports are the signal
        str_sda = s[-SDA_BITID-1:-SDA_BITID] # extract char
        str_scl = s[-SCL_BITID-1:-SCL_BITID] # extract char
        self._dut._log.info(f"signal={type(signal)} s={s} str_sda={str_sda} str_scl={str_scl}")
        if SCL_BITID < SDA_BITID: # REMOVE ME
            r = s[-4:-2] # SDA SCL old mapping uio[3:2]
        else: # REMOVE ME
            r = s[-5:-3] # SCL SDA new mapping uio[4:3]
        # "xxx10xxx" => "01" SDA SCL new (will get transposed automatically as we picked out each bit and reformed)
        # "xxxx01xx" => "01" SDA SCL old
        rr = str_sda + str_scl
        #assert rr == r, f"{str_sda + str_scl} != {r}" # obviously always an error
        assert rr[0] == r[1], f"{str_sda + str_scl} != {r}"
        assert rr[1] == r[0], f"{str_sda + str_scl} != {r}"
        return rr
    ...
    def sda_rx(self) -> bool:
        if not self._sdascl_out.raw.value.is_resolvable:
            ...
            x = self.signal_to_string(self._sdascl_out.raw.value) # "01" SDA SCL
            my_sda = x[0]		# SDA is 1st char
            if my_sda == 'x':
                ...
                my_sda = '1'
            assert my_sda == '0' or my_sda == '1', f"my_sda={str(self._sdascl_out.raw.value)}" # message not transposed
            nv = True if my_sda == '1' else False
            return nv
        return (self._sdascl_out.value & self._sda_int_mask) != 0
    ...
    def sda_oe(self) -> bool:
        if not self._sdascl_oe.raw.value.is_resolvable:
            #nv = False	# FIXME pickup RANDOM_POLICY
            x = self.signal_to_string(self._sdascl_oe.raw.value) # "01" SDA SCL
            my_sda_oe = x[0]		# SDA is 1st char
            if my_sda_oe == 'x':
                ...
                my_sda_oe = '1'
            assert my_sda_oe == '0' or my_sda_oe == '1', f"my_sda_oe={str(self._sdascl_oe.raw.value)}" # message not transposed
            nv = True if my_sda_oe == '1' else False
            return nv
        return (self._sdascl_oe.value & self._sda_int_mask) != 0
    ...
    def resolve_bits_state_str(self, sda: bool, scl: bool, transpose: bool, when_none: str = None) -> str:
        ...
        if transpose:
            return scl_c + sda_c # new
        else:
            return sda_c + scl_c # old
    ...
    def set_sda_raw_and_scl_raw(self, sda: bool = None, scl: bool = None) -> None:
        ...
        v = self.resolve_bits_state_str(sda, scl, self._sdascl_transpose)
